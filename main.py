...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Safe TMDB GET using requests + asyncio.to_thread
    (httpx has ConnectError on Windows, so we use requests in a thread).
    """
    q = dict(params)
    q["api_key"] = TMDB_API_KEY

    def _do_request():
        return req_lib.get(f"{TMDB_BASE}{path}", params=q, timeout=20)
