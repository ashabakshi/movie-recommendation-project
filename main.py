...
def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Safe TMDB GET using sync requests. FastAPI automatically 
    routes this to a threadpool, preventing blocking.
    """
    q = dict(params)
    q["api_key"] = TMDB_API_KEY

    try:
        r = requests.get(f"{TMDB_BASE}{path}", params=q, timeout=15)
...
def tmdb_cards_from_results(results: List[dict], limit: int = 20) -> List[TMDBMovieCard]:
...
# =========================
# ROUTES (FastAPI sync route handlers)
# =========================
@app.get("/health")
def health():
    return {"status": "ok"}
